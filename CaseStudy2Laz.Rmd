---
title: "CaseStudy2 Work Place Attrition & Salary"
...
df <- read.csv("https://raw.githubusercontent.com/JosephLazarus/CaseStudy2DDS_Work_Place_Attrition/main/CaseStudy2-data.csv")
...
```{r}
df$ntime.at.past.job <- normalize(df$time.at.past.job,  method = "standardize", on.constant = "quiet")
...
dat <- select(df, c("Attrition", "Age",'NumCompaniesWorked', 'ntime.at.past.job', 
  'DistanceFromHome', 'EnvironmentSatisfaction','JobInvolvement', 
  'JobSatisfaction',  'RelationshipSatisfaction', 'StockOptionLevel',  'TotalWorkingYears', 'TrainingTimesLastYear',  'WorkLifeBalance', 'YearsInCurrentRole',  'YearsSinceLastPromotion', 'YearsWithCurrManager',  'iJobRole', 'iDepartment', "iMaritalStatus"))
...
Knn_pred <- predict(KnnModel, test[-1])
#Print confusion matrix
Knn.CM <- confusionMatrix(Knn_pred, as.factor(test$Attrition))
Knn.CM
...
{
  trainIndices2 <- sample(1:dim(dat)[1],round(splitPerc * dim(dat)[1]))
  train.k2 <- dat[trainIndices2,]
  test.k2 <- dat[-trainIndices2,]
  
  for(i in 1:numks)
  {
    classifications = knn(train.k2[,2:19],test.k2[,2:19],train.k2$Attrition ,k=i,prob = TRUE)
    table(test.k2$Attrition ,classifications)
    CM = confusionMatrix(table(test.k2$Attrition,classifications))
...
max(MeanSens)
which.max(MeanSpec)
max(na.omit(MeanSpec)) #best sensitivity i could produce us 0.658

set.seed(29)


classifications = knn(train.k2[,2:19], test.k2[,2:19], train.k2$Attrition,
                      k=9, prob = TRUE)
table(test.k2$Attrition, classifications)
...
library(e1071)
#Fit the model on train data
NBModel <- naiveBayes(Attrition ~., data=training, laplace = -1, usekernel = TRUE)
...
Specificities <- c(DTM.CM$byClass["Specificity"], GLM.CM$byClass["Specificity"], 
                   ...
                   RFM.CM$byClass["Specificity"])

Precisions <- c(DTM.CM$byClass["Precision"], GLM.CM$byClass["Precision"], 
                NBM.CM$byClass["Precision"], KNN.CM$byClass["Precision"],
                RFM.CM$byClass["Percision"])

Recalls <- c(DTM.CM$byClass["Recall"], GLM.CM$byClass["Recall"], 
             NBM.CM$byClass["Recall"], KNN.CM$byClass["Recall"], RFM.CM$byClass["Recall"])

Accuracies <- c(DTM.CM$overall[1], GLM.CM$overall[1], NBM.CM$overall[1], KNN.CM$overall[1], 
                RFM.CM$overall[1])

Balanced_Accuracies <- c(DTM.CM$byClass["Balanced Accuracy"],GLM.CM$byClass["Balanced Accuracy"], 
                         NBM.CM$byClass["Balanced Accuracy"], KNN.CM$byClass["Balanced Accuracy"],
                         RFM.CM$byClass["Balanced Accuracy"])

F1_Scores <- c(DTM.CM$byClass["F1"], GLM.CM$byClass["F1"], NBM.CM$byClass["F1"], 
               KNN.CM$byClass["F1"], RFM.CM$byClass["F1"])

#Create a data frame from the metrics in confusion matrixes and models
Models  <- c("DTM", "GLM", "NBM", "KNN", "RFM")
...
# The KNN model is the only one that meets the requirements of 60% / %60 I thought that the Naive Bayes model was doing a better. But this was helpful to see it on graph form. 
sn.p
sp.p
pr.p
ba.p
...
attritionCompSet <- read.csv("https://raw.githubusercontent.com/JosephLazarus/CaseStudy2DDS_Work_Place_Attrition/main/CaseStudy2CompSet%20No%20Attrition.csv")

# Next steps are to process the data the same way i did before. Though I am not going to make the group factors since I did not find them to be useful. In my models

str(attritionCompSet)
summary(attritionCompSet)
dim(attritionCompSet) # 300 X 35

which(is.na(attritionCompSet)) # makes sure no funny business is going here

colSums(is.na(attritionCompSet)) # still 0

attritionCompSet <- select(attritionCompSet, -c(Over18, EmployeeCount, StandardHours, EmployeeNumber))
dim(attritionCompSet) #300 X 31


which(is.na(attritionCompSet)) # double check 
...
str(attritionCompSet)
colSums(is.na(attritionCompSet))

#calculating the time at previous job column
...
library(BBmisc)
attritionCompSet$ntime.at.past.job <- normalize(attritionCompSet$time.at.past.job,  method = "standardize", on.constant = "quiet")


attritionPredictionData <- select(attritionCompSet, c("ID","Age",'NumCompaniesWorked', 'ntime.at.past.job','DistanceFromHome', 'EnvironmentSatisfaction','JobInvolvement', 'JobSatisfaction', 'RelationshipSatisfaction','StockOptionLevel','TotalWorkingYears', 'TrainingTimesLastYear', 'WorkLifeBalance', 'YearsInCurrentRole', 'YearsSinceLastPromotion','YearsWithCurrManager','iJobRole', 'iDepartment', "iMaritalStatus"))

colnames(attritionPredictionData)
summary(attritionPredictionData)
...
library(class) #bring that library back in case randomForest masked it

#using the KNN model on the entire
predictAttrit = knn(dat[,2:19],attritionPredictionData[,2:19], dat$Attrition, k=9, prob= TRUE)

#saving the object as a back up
Attrition <- predictAttrit
...
case2predictions <- select(case2predictions, -c("Age",'NumCompaniesWorked', 'ntime.at.past.job','DistanceFromHome', 'EnvironmentSatisfaction','JobInvolvement', 'JobSatisfaction', 'RelationshipSatisfaction','StockOptionLevel', 'TotalWorkingYears', 'TrainingTimesLastYear', 'WorkLifeBalance', 'YearsInCurrentRole', 
'YearsSinceLastPromotion','YearsWithCurrManager','iJobRole', 'iDepartment', "iMaritalStatus"))

#write it a csv in submission format
write.csv(case2predictions,"Case2PredictionsLAZARUSAttrition.csv", row.names = FALSE)
...
saldat <- select(df, "MonthlyIncome", "Age", "Attrition", "BusinessTravel", "YearsAtCompany", "YearsInCurrentRole", "TotalWorkingYears", "JobLevel", "JobRole", "Department")
...
salaryCompset <- read.csv("https://raw.githubusercontent.com/JosephLazarus/CaseStudy2DDS_Work_Place_Attrition/main/CaseStudy2CompSet%20No%20Salary.csv")
