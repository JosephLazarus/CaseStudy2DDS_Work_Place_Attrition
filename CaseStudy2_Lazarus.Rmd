...
dat <- select(df, c("Attrition", "Age",'NumCompaniesWorked', 'ntime.at.past.job', 'DistanceFromHome', 'EnvironmentSatisfaction','JobInvolvement', 'JobSatisfaction', 'RelationshipSatisfaction','StockOptionLevel','TotalWorkingYears','TrainingTimesLastYear', 'WorkLifeBalance', 'YearsInCurrentRole',  'YearsSinceLastPromotion','YearsWithCurrManager','iJobRole', 'iDepartment', "iMaritalStatus"))
...
# confusion matrix
DTM.CM <- confusionMatrix(DTModel_pred, as.factor(test$iAttrition))
...
max(MeanSens)
which.max(MeanSpec)
max(na.omit(MeanSpec)) #best sensitivity i could produce us 0.658 k=9
